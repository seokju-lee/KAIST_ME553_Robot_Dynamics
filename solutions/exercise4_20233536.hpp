...
#ifndef ME553_2023_SOLUTIONS_EXERCISE4_20233536_HPP_
#define ME553_2023_SOLUTIONS_EXERCISE4_20233536_HPP_
...
class Body {
  public:
    struct Joint{
      ...
      std::vector<Eigen::MatrixXd> inertia_C, Si;
      std::vector<Eigen::VectorXd> sA, sF, sP;
      Eigen::MatrixXd jointI, motion_W;
      Eigen::VectorXd motion_P, v, a, nonlinear, ff;
    ...
    Eigen::MatrixXd crm(const Eigen::VectorXd& c) {
      Eigen::MatrixXd skew_symmetric(6,6);
      skew_symmetric << skew(c.tail(3)), Eigen::MatrixXd::Zero(3,3),
                        skew(c.head(3)), skew(c.tail(3));
      return skew_symmetric;
    } 

    void computeKinematicsDownTheTree(const Eigen::VectorXd& gc, const Eigen::VectorXd& gv, std::vector<Eigen::VectorXd> sA, std::vector<Eigen::VectorXd> sP, std::vector<Eigen::VectorXd> sF, std::vector<Eigen::MatrixXd> si) {
      ...
      if (joint_.type == Joint::Type::FLOATING) {
        ...
        // save the set for acceleation, fictitious force, position, and motion subspace
        ...
        // when the compute down the tree for first leg
          ...
          joint_.sA.push_back(joint_.jointI*joint_.a);
      ...
      for (auto& par: parent_) {
        par->computeKinematicsDownTheTree(gc, gv, sA, sP, sF, si);
...
inline Eigen::VectorXd getNonlinearities (const Eigen::VectorXd& gc, const Eigen::VectorXd& gv) {
  ...
  std::vector<Eigen::VectorXd> sA, sP, sF;
  std::vector<Eigen::MatrixXd> si;
  ...
  FR_calf_foot.computeKinematicsDownTheTree(gc, gv, sA, sP, sF, si);
  FL_calf_foot.computeKinematicsDownTheTree(gc, gv, FR_calf_foot.joint_.sA, FR_calf_foot.joint_.sP, FR_calf_foot.joint_.sF, FR_calf_foot.joint_.Si);
  RR_calf_foot.computeKinematicsDownTheTree(gc, gv, FL_calf_foot.joint_.sA, FL_calf_foot.joint_.sP, FL_calf_foot.joint_.sF, FL_calf_foot.joint_.Si);
  RL_calf_foot.computeKinematicsDownTheTree(gc, gv, RR_calf_foot.joint_.sA, RR_calf_foot.joint_.sP, RR_calf_foot.joint_.sF, RR_calf_foot.joint_.Si);

  // compute backward recursive for final leg to obtain the nonlinearities
  RL_calf_foot.computebackrecursive();

  return RL_calf_foot.joint_.nonlinear;
}


#endif // ME553_2023_SOLUTIONS_EXERCISE4_20233536_HPP_